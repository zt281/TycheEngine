...
class TestHandleFill:
    """Tests for on_broadcasted_fill behavior."""

    def test_buy_fill_increases_long_position(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
    ...
    def test_second_buy_fill_updates_avg_entry_price(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
        portfolio.on_broadcasted_fill(_fill("o2", "BTC-USD", Side.BUY, 200, 10).to_dict())
        pos = portfolio.get_position("BTC-USD")
    ...
    def test_sell_fill_decreases_long_position(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
        portfolio.on_broadcasted_fill(_fill("o2", "BTC-USD", Side.SELL, 120, 6).to_dict())
        pos = portfolio.get_position("BTC-USD")
        assert pos.side == PositionSide.LONG
    ...
    def test_sell_fill_creates_short_when_exceeds_long(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
        portfolio.on_broadcasted_fill(_fill("o2", "BTC-USD", Side.SELL, 120, 15).to_dict())
        pos = portfolio.get_position("BTC-USD")
...
class TestHandleQuote:
    ...
    def test_quote_publishes_only_if_pnl_change_exceeds_threshold(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
        with patch.object(portfolio, "send_event") as mock_send:
            portfolio.on_streaming_quote({"instrument_id": "BTC-USD", "bid": "100.001", "ask": "100.001"})
            mock_send.assert_not_called()

    def test_quote_publishes_when_pnl_change_is_material(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
        with patch.object(portfolio, "send_event") as mock_send:
            portfolio.on_streaming_quote({"instrument_id": "BTC-USD", "bid": "110", "ask": "110"})
            mock_send.assert_called_once()

    def test_quote_ignores_unknown_instrument(self, portfolio):
        with patch.object(portfolio, "send_event") as mock_send:
            portfolio.on_streaming_quote({"instrument_id": "ETH-USD", "bid": "200", "ask": "202"})
            mock_send.assert_not_called()
...
class TestPnlProperties:
    ...
    def test_total_realized_pnl(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
        portfolio.on_broadcasted_fill(_fill("o2", "BTC-USD", Side.SELL, 120, 10).to_dict())
    ...
    def test_pnl_across_multiple_instruments(self, portfolio):
        portfolio.on_broadcasted_fill(_fill("o1", "BTC-USD", Side.BUY, 100, 10).to_dict())
        portfolio.on_broadcasted_fill(_fill("o2", "ETH-USD", Side.BUY, 50, 5).to_dict())
        portfolio.on_streaming_quote({"instrument_id": "BTC-USD", "bid": "110", "ask": "110"})
        portfolio.on_streaming_quote({"instrument_id": "ETH-USD", "bid": "55", "ask": "55"})
