...
def test_interface_pattern_values():
    """InterfacePattern enum has v3 ON, SEND, HANDLE, REQUEST."""
    assert InterfacePattern.ON.value == "on"
    assert InterfacePattern.SEND.value == "send"
    assert len(InterfacePattern) == 4
...
def test_interface_pattern_handle_value():
    """InterfacePattern.HANDLE == 'handle' for job handler declaration."""
    assert InterfacePattern.HANDLE.value == "handle"


def test_interface_pattern_request_value():
    """InterfacePattern.REQUEST == 'request' for job requester declaration."""
    assert InterfacePattern.REQUEST.value == "request"


def test_message_type_request_value():
    """MessageType.REQUEST == 'req' for job request messages."""
