"""Unit tests for tyche_client.protocol constants."""

import pytest


def test_message_types_are_bytes():
    from tyche_client.protocol import READY, ACK, HB, CMD, REPLY, DISCO
...
def test_command_types_are_bytes():
    from tyche_client.protocol import CMD_START, CMD_STOP, CMD_RECONFIGURE, CMD_STATUS
...
def test_status_codes_are_bytes():
    from tyche_client.protocol import STATUS_OK, STATUS_ERROR
...
def test_protocol_version_is_int():
    from tyche_client.protocol import PROTOCOL_VERSION
...
def test_default_timeouts_are_ints():
    from tyche_client.protocol import (
