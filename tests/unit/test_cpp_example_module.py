"""Tests for CppExampleModule - verifies parity with Python ExampleModule."""
...
def test_cpp_example_module_init():
    """CppExampleModule generates an example-prefixed module ID."""
    module = CppExampleModule(engine_endpoint=Endpoint("127.0.0.1", 5555))
    assert module.module_id.startswith("example")


def test_cpp_example_module_discovers_v3_interfaces():
    """CppExampleModule only registers signal pair handlers (on_ping, on_pong)."""
    module = CppExampleModule(engine_endpoint=Endpoint("127.0.0.1", 5555))
